package Programas;

import java.io.IOException;
import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import Clases.Detalle;
import Conexiones.DetalleDAO;

/**
 * Servlet implementation class ActualizarDetalle
 */
@WebServlet("/ActualizarDetalle")
public class ActualizarDetalle extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ActualizarDetalle() {
        super();
        // TODO Auto-generated constructor stub
    }

	/**
	 * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// TODO Auto-generated method stub
		response.getWriter().append("Served at: ").append(request.getContextPath());
	}

	/**
	 * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response)
	 */
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		int detalleId = Integer.parseInt(request.getParameter("detalleId"));
		int unidades = Integer.parseInt(request.getParameter("unidades"));
		
		DetalleDAO detalleDAO = new DetalleDAO();
		Detalle detalle = detalleDAO.obtenerDetallePorId(detalleId);
		
		if (detalle != null) {
			detalle.setUnidades(unidades);
			double total = (detalle.getPreciounidad() * unidades) + (detalle.getPreciounidad() * unidades * detalle.getImpuesto() / 100);
			detalle.setTotal(total);
			detalleDAO.actualizarDetalle(detalle);
		}
		
		response.sendRedirect("Carrito.jsp");
	}

}
